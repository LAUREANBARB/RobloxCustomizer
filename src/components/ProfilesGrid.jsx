            ...
            return (
              <div key={profile.name} className={`card-stagger card p-5 group relative ${isActive ? 'card-active' : ''}`} style={{ animationDelay: `${i * 0.1}s` }}>
                ...
                <div className="space-y-1.5 mb-4">
                  ...
                  {profile.font && <div className="flex items-center gap-2 text-xs"><span style={{ color: 'var(--text-dim)' }}>Font</span><span style={{ color: 'var(--text-secondary)' }}>{profile.font}</span></div>}
