  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2">
      {notifications.map((n) => (
        <div
          key={n.id}
          className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl border backdrop-blur-xl text-sm font-medium ${n.exiting ? 'notif-exit' : 'notif-enter'} ${typeStyles[n.type] || typeStyles.info} ${n.action ? 'cursor-pointer hover:brightness-125' : ''}`}
          onClick={() => n.action?.()}
        >
